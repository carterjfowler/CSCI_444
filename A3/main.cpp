...
CSCI441::ShaderProgram *blinnPhongShaderProgram = nullptr;

CSCI441::UniformBufferObject *blinnPhongLightUBO = nullptr;
CSCI441::UniformBufferObject *blinnPhongMaterialUBO = nullptr;
CSCI441::UniformBufferObject *blinnPhongVertUBO = nullptr;
struct BlinnPhongShaderAttributes {
    GLint vPos;                                 // Vertex Position
    GLint vNorm;
} blinnPhongShaderAttributes;

//Trefoil variables
GLint vpos_location, vnorm_location;

const int SLICES = 128;
const int STACKS = 32;
const int VERTEX_COUNT = SLICES * STACKS;
...
GLuint vao_trefoil, vertex_buffer_trefoil, element_buffer_trefoil;
...
GLFWwindow* setupGLFW() {
    ...
    GLFWwindow *window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "A3: Particle System", nullptr, nullptr );
...
void setupShaders() {
    ...
    vpos_location = colorAttribLocations.vPos;
    vnorm_location = colorAttribLocations.vColor;

    blinnPhongShaderProgram = new CSCI441::ShaderProgram("shaders/blinnPhongShader.v.glsl", "shaders/blinnPhongShader.f.glsl");

    blinnPhongShaderAttributes.vPos          = blinnPhongShaderProgram->getAttributeLocation("VertexPosition" );
    blinnPhongShaderAttributes.vNorm          = blinnPhongShaderProgram->getAttributeLocation("VertexNormal" );
...
//	peashooter = new CSCI441::ModelLoader("assets/models/peashooter.obj");
    ...
    glGenVertexArrays(1, &vao_trefoil );
    glBindVertexArray(vao_trefoil);

    blinnPhongLightUBO = new CSCI441::UniformBufferObject("LightInfo",
                                                          {"LightInfo.Position", "LightInfo.La", "LightInfo.Ld", "LightInfo.Ls"});
    blinnPhongLightUBO->setupWithShaderProgram(blinnPhongShaderProgram, 0 );

    blinnPhongMaterialUBO = new CSCI441::UniformBufferObject("MaterialInfo",
                                                             {"MaterialInfo.Ka", "MaterialInfo.Kd", "MaterialInfo.Ks",
                                                          "MaterialInfo.Shininess"});
    blinnPhongMaterialUBO->setupWithShaderProgram(blinnPhongShaderProgram, 1 );

    blinnPhongVertUBO = new CSCI441::UniformBufferObject("VertInfo",
                                                         {"VertInfo.ModelViewMatrix", "VertInfo.NormalMatrix",
                                                      "VertInfo.ProjectionMatrix", "VertInfo.MVP"});
    blinnPhongVertUBO->setupWithShaderProgram(blinnPhongShaderProgram, 2 );

    vertex_buffer_trefoil = CreateVertexBuffer();
    element_buffer_trefoil = CreateIndexBuffer();
}
...
GLuint CreateVertexBuffer() {
    ...
    for (float s = 0; s < 1 - ds / 2; s += ds) {
        for (float t = 0; t < 1 - dt / 2; t += dt) {
            const float E = 0.01f;
            glm::vec3 p = EvaluateTrefoil(s, t);
            glm::vec3 u = EvaluateTrefoil(s + E, t) - p;
            glm::vec3 v = EvaluateTrefoil(s, t + E) - p;
    ...
    if(pVert - &verts[0] == VERTEX_COUNT*6*sizeof(GLfloat))
        printf( "[ERROR]: Tessellation error pVert-&verts[0] == VERTEX_COUNT*6*sizeof(GLfloat)\n");
    ...
    glVertexAttribPointer( vnorm_location, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 6, (void*)(3*sizeof(float)));

    return handle;
}

GLuint CreateIndexBuffer() {
    GLushort inds[INDEX_COUNT];
    ...
    for (GLushort i = 0; i < SLICES; i++) {
        for (GLushort j = 0; j < STACKS; j++) {
            *pIndex++ = n + j;
            *pIndex++ = n + (j + 1) % STACKS;
            *pIndex++ = (n + j + STACKS) % VERTEX_COUNT;

            *pIndex++ = (n + j + STACKS) % VERTEX_COUNT;
            *pIndex++ = (n + (j + 1) % STACKS) % VERTEX_COUNT;
            *pIndex++ = (n + (j + 1) % STACKS + STACKS) % VERTEX_COUNT;
        }
        n += STACKS;
    }

    if(n != VERTEX_COUNT)
        printf( "[ERROR]: Tessellation error n != VERTEX_COUNT (%d != %d)\n", n, VERTEX_COUNT);
    if (pIndex - &inds[0] == INDEX_COUNT*sizeof(GLushort))
        printf( "[ERROR]: Tessellation error pIndex - &inds[0] == INDEX_COUNT*sizeof(GLushort)\n");

    GLuint handle;

    glGenBuffers(1, &handle);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, handle);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(inds), &inds[0], GL_STATIC_DRAW);
...
void setupScene() {
    ...
    blinnPhongLightUBO->copyToOffset(0, (void*)(&LightPos[0]), sizeof(glm::vec4) );
    blinnPhongLightUBO->copyToOffset(1, (void*)(&LightAmb[0]), sizeof(glm::vec3) );
    blinnPhongLightUBO->copyToOffset(2, (void*)(&LightDiff[0]), sizeof(glm::vec3) );
    blinnPhongLightUBO->copyToOffset(3, (void*)(&LightSpec[0]), sizeof(glm::vec3) );
    blinnPhongLightUBO->bindBuffer();
    blinnPhongLightUBO->bufferSubData();
}
...
void renderScene( const glm::mat4 VIEW_MTX, const glm::mat4 PROJ_MTX ) {
    ...
    // ------ BEGIN draw object ------
    glm::vec3 MaterAmb = glm::vec3(0.0, 0.0, 0.3);
    glm::vec3 MaterDiff = glm::vec3(0.0, 0.0, 1.0);
    glm::vec3 MaterSpec = glm::vec3(0.0, 0.0, 0.7);
...
//    glUniformMatrix4fv( colorUniformLocations.mvpMtx, 1, GL_FALSE, &mvpMtx[0][0] );
    blinnPhongMaterialUBO->copyToOffset(0, (void*)(&MaterAmb[0]), sizeof(glm::vec3) );
    blinnPhongMaterialUBO->copyToOffset(1, (void*)(&MaterDiff[0]), sizeof(glm::vec3) );
    blinnPhongMaterialUBO->copyToOffset(2, (void*)(&MaterSpec[0]), sizeof(glm::vec3) );
    blinnPhongMaterialUBO->copyToOffset(3, &shininess, sizeof(GLfloat) );
    blinnPhongMaterialUBO->bindBuffer();
    blinnPhongMaterialUBO->bufferSubData();

    blinnPhongVertUBO->copyToOffset(0, (void*)(&mvMtx[0][0]), sizeof(glm::mat4) );
    blinnPhongVertUBO->copyToOffset(1, (void*)(&normalMtx[0][0]), sizeof(glm::mat3) );
    blinnPhongVertUBO->copyToOffset(2, (void*)(&PROJ_MTX[0][0]), sizeof(glm::mat4) );
    blinnPhongVertUBO->copyToOffset(3, (void*)(&mvpMtx[0][0]), sizeof(glm::mat4) );
    blinnPhongVertUBO->bindBuffer();
    blinnPhongVertUBO->bufferSubData();
    glBindVertexArray(vao_trefoil);
