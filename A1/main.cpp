...
CSCI441::UniformBufferObject *gouradLightUBO = nullptr;
CSCI441::UniformBufferObject *gouradMaterialUBO = nullptr;
CSCI441::UniformBufferObject *gouradVertUBO = nullptr;
//CSCI441::UniformBufferObject *phongVertUBO = nullptr;
//CSCI441::UniformBufferObject *phongLightUBO = nullptr;
//CSCI441::UniformBufferObject *phongMaterialUBO = nullptr;

//GLint gouradLightBlockIndex = 0, gouradMaterialBlockIndex = 0, gouradVertBlockIndex = 0;
//GLint phongVertBlockIndex = 0, phongLightBlockIndex = 0, phongMaterialBlockIndex = 0;
//
//GLint gouradLightBlockSize = 0, gouradMaterialBlockSize = 0, gouradVertBlockSize = 0;
//GLint phongVertBlockSize = 0, phongLightBlockSize = 0, phongMaterialBlockSize = 0;
//
//GLubyte *gouradLightBlockBuffer = nullptr, *gouradMaterialBlockBuffer = nullptr,
//*gouradVertBlockBuffer = nullptr;
//GLubyte *phongVertBlockBuffer = nullptr, *phongLightBlockBuffer = nullptr,
//*phongMaterialBlockBuffer = nullptr;
//
//GLint gouradLightOffsets[4], gouradMaterialOffsets[4], gouradVertOffsets[4];
//GLint phongLightOffsets[4], phongMaterialOffsets[4], phongVertOffsets[4];

struct GouradShaderAttributes {
    GLint vPos;                                 // Vertex Position
    GLint vNorm;
} gouradShaderAttributes;
struct PhongShaderAttributes {
    GLint vPos;                                 // Vertex Position
    GLint vNorm;
} phongShaderAttributes;

struct GouradShaderSubroutines {
    GLuint vertex, vGouradPhong, vGouradBlinnPhong, vPhongPassThrough;
    GLuint fragment, fPhongPhong, fPhongBlinnPhong;
} gouradShaderSubroutineIndices;
//struct PhongShaderSubroutines {
//    GLuint fragment, fPhong, fBlinnPhong;
//} phongShaderSubroutineIndices;

bool gouradShaderUsed = true, phongShading = true;
    ...
    if(action == GLFW_PRESS) {
        switch(key) {
            ...
            case GLFW_KEY_Q:
                ...
                break;
            case GLFW_KEY_1:
                gouradShaderUsed = true;
                break;
            case GLFW_KEY_2:
                gouradShaderUsed = false;
                break;
            case GLFW_KEY_3:
                phongShading = true;
                break;
...
GLFWwindow* setupGLFW() {
    ...
    GLFWwindow *window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "A1: OpenGL 4.1 Context", nullptr, nullptr );
...
void setupShaders() {
    ...
    gouradShaderAttributes.vPos          = gouradShaderProgram->getAttributeLocation( "VertexPosition" );
    gouradShaderAttributes.vNorm          = gouradShaderProgram->getAttributeLocation( "VertexNormal" );

    gouradShaderSubroutineIndices.vGouradPhong = gouradShaderProgram->getSubroutineIndex( GL_VERTEX_SHADER, "gouradPhongModel" );
    gouradShaderSubroutineIndices.vGouradBlinnPhong = gouradShaderProgram->getSubroutineIndex( GL_VERTEX_SHADER, "gouradBlinnPhong" );
    gouradShaderSubroutineIndices.vPhongPassThrough = gouradShaderProgram->getSubroutineIndex( GL_VERTEX_SHADER, "phongPassThrough" );
    gouradShaderSubroutineIndices.vertex = gouradShaderSubroutineIndices.vGouradPhong;
    gouradShaderSubroutineIndices.fPhongPhong = gouradShaderProgram->getSubroutineIndex( GL_FRAGMENT_SHADER, "phongPhongModel" );
    gouradShaderSubroutineIndices.fPhongBlinnPhong = gouradShaderProgram->getSubroutineIndex( GL_FRAGMENT_SHADER, "phongBlinnPhong" );
    gouradShaderSubroutineIndices.fragment = gouradShaderSubroutineIndices.fPhongPhong;


//    gouradLightBlockIndex = glGetUniformBlockIndex(gouradShaderProgram->getShaderProgramHandle(), "LightInfo");
//    gouradMaterialBlockIndex = glGetUniformBlockIndex(gouradShaderProgram->getShaderProgramHandle(), "MaterialInfo");
//    gouradVertBlockIndex = glGetUniformBlockIndex(gouradShaderProgram->getShaderProgramHandle(), "VertInfo");
//
//    glGetActiveUniformBlockiv(gouradShaderProgram->getShaderProgramHandle(), gouradLightBlockIndex,
//                              GL_UNIFORM_BLOCK_DATA_SIZE, &gouradLightBlockSize);
//    glGetActiveUniformBlockiv(gouradShaderProgram->getShaderProgramHandle(), gouradMaterialBlockIndex,
//                              GL_UNIFORM_BLOCK_DATA_SIZE, &gouradMaterialBlockSize);
//    glGetActiveUniformBlockiv(gouradShaderProgram->getShaderProgramHandle(), gouradVertBlockIndex,
//                              GL_UNIFORM_BLOCK_DATA_SIZE, &gouradVertBlockSize);

//    phongShaderProgram = new CSCI441::ShaderProgram("shaders/phongShader.v.glsl", "shaders/phongShader.f.glsl");
//
//    phongShaderAttributes.vPos          = phongShaderProgram->getAttributeLocation( "VertexPosition" );
//    phongShaderAttributes.vNorm          = phongShaderProgram->getAttributeLocation( "VertexNormal" );
//
//    phongShaderSubroutineIndices.fPhong = phongShaderProgram->getSubroutineIndex( GL_FRAGMENT_SHADER, "phongModel" );
//    phongShaderSubroutineIndices.fBlinnPhong = phongShaderProgram->getSubroutineIndex( GL_FRAGMENT_SHADER, "blinnPhong" );
//    phongShaderSubroutineIndices.fragment = phongShaderSubroutineIndices.fPhong;
//
//    phongVertBlockIndex = glGetUniformBlockIndex(phongShaderProgram->getShaderProgramHandle(), "VertInfo");
//    phongLightBlockIndex = glGetUniformBlockIndex(phongShaderProgram->getShaderProgramHandle(), "LightInfo");
//    phongMaterialBlockIndex = glGetUniformBlockIndex(phongShaderProgram->getShaderProgramHandle(), "MaterialInfo");
//
//    glGetActiveUniformBlockiv(phongShaderProgram->getShaderProgramHandle(), phongVertBlockIndex,
//                              GL_UNIFORM_BLOCK_DATA_SIZE, &phongVertBlockSize);
//    glGetActiveUniformBlockiv(phongShaderProgram->getShaderProgramHandle(), phongLightBlockIndex,
//                              GL_UNIFORM_BLOCK_DATA_SIZE, &phongLightBlockSize);
//    glGetActiveUniformBlockiv(phongShaderProgram->getShaderProgramHandle(), phongMaterialBlockIndex,
//                              GL_UNIFORM_BLOCK_DATA_SIZE, &phongMaterialBlockSize);
}
...
void setupBuffers() {
    ...
    const glm::vec3 vertexNormals[NUM_CUBE_VERTICES] = {
            { (  faceNorm[0] + faceNorm[3] + faceNorm[5]) / 3.0f }, // 0 - bln
            { (  faceNorm[0] + faceNorm[3] + faceNorm[4]) / 3.0f }, // 1 - brn
            { (  faceNorm[0] + faceNorm[2] + faceNorm[4]) / 3.0f }, // 2 - trn
            { (  faceNorm[0] + faceNorm[2] + faceNorm[5]) / 3.0f }, // 3- tln
            { (  faceNorm[1] + faceNorm[3] + faceNorm[5]) / 3.0f }, // 4- blf
            { (  faceNorm[1] + faceNorm[3] + faceNorm[4]) / 3.0f }, // 5- brf
            { (  faceNorm[1] + faceNorm[3] + faceNorm[4]) / 3.0f }, // 6- trf
            { (  faceNorm[1] + faceNorm[2] + faceNorm[5]) / 3.0f }, // 7- tlf
    };
    const GouradVertex cubeVertices[NUM_CUBE_VERTICES] = {
            { -0.5f, -0.5f, -0.5f, vertexNormals[0] }, // 0 - bln
            {  0.5f, -0.5f, -0.5f, vertexNormals[1] }, // 1 - brn
            {  0.5f,  0.5f, -0.5f, vertexNormals[2] }, // 2 - trn
            { -0.5f, 0.5f, -0.5f, vertexNormals[3] }, // 3- tln
            { -0.5f, -0.5f, 0.5f, vertexNormals[4] }, // 4- blf
            { 0.5f, -0.5f, 0.5f, vertexNormals[5] }, // 5- brf
            { 0.5f, 0.5f, 0.5f, vertexNormals[6] }, // 6- trf
            { -0.5f, 0.5f, 0.5f, vertexNormals[7] }, // 7- tlf
    };
    ...
    /*Gourad UBO Declarations Start*/
    gouradLightUBO = new CSCI441::UniformBufferObject("LightInfo",
        {"LightInfo.Position", "LightInfo.La", "LightInfo.Ld", "LightInfo.Ls"});
    gouradLightUBO->setupWithShaderProgram( gouradShaderProgram, 0 );

    gouradMaterialUBO = new CSCI441::UniformBufferObject("MaterialInfo",
        {"MaterialInfo.Ka", "MaterialInfo.Kd", "MaterialInfo.Ks",
         "MaterialInfo.Shininess"});
    gouradMaterialUBO->setupWithShaderProgram( gouradShaderProgram, 1 );

    gouradVertUBO = new CSCI441::UniformBufferObject("VertInfo",
        {"VertInfo.ModelViewMatrix", "VertInfo.NormalMatrix",
         "VertInfo.ProjectionMatrix", "VertInfo.MVP"});
    gouradVertUBO->setupWithShaderProgram( gouradShaderProgram, 2 );
    /*Gourad UBO Declarations End*/

    /*Phong UBO Declarations Start*/
//    phongVertUBO = new CSCI441::UniformBufferObject("VertInfo",
//        {"ModelViewMatrix", "ProjectionMartrix", "MVP", "NormalMatrix"});
//    phongVertUBO->setupWithShaderProgram( phongShaderProgram, 3 );
//
//    phongLightUBO = new CSCI441::UniformBufferObject("LightInfo",
//        {"Position", "La", "Ld", "Ls"});
//    phongLightUBO->setupWithShaderProgram( phongShaderProgram, 4 );
//
//    phongMaterialUBO = new CSCI441::UniformBufferObject("MaterialInfo",
//        {"Ka", "Kd", "Ks", "Shininess"});
//    phongMaterialUBO->setupWithShaderProgram( phongShaderProgram, 5 );
    /*Phong UBO Declarations End*/


//    gouradLightBlockBuffer = (GLubyte*)malloc(gouradLightBlockSize);
//    gouradMaterialBlockBuffer = (GLubyte*)malloc(gouradMaterialBlockSize);
//    gouradVertBlockBuffer = (GLubyte*)malloc(gouradVertBlockSize);
//    phongVertBlockBuffer = (GLubyte*)malloc(phongVertBlockSize);
//    phongLightBlockBuffer = (GLubyte*)malloc(phongLightBlockSize);
//    phongMaterialBlockBuffer = (GLubyte*)malloc(phongMaterialBlockSize);
//
//    GLchar *gouradLightNames[4] = {"Position", "La", "Ld", "Ls"};
//    GLchar *gouradMaterialNames[4] = {"Ka", "Kd", "Ks", "Shininess"};
//    GLchar *gouradVertNames[4] = {"ModelViewMatrix", "NormalMatrix", "ProjectionMatrix", "MVP"};
//
//    GLchar *phongVertNames[4] = {"ModelViewMatrix", "ProjectionMartrix", "MVP", "NormalMatrix"};
//    GLchar *phongLightNames[4] = {"Position", "La", "Ld", "Ls"};
//    GLchar *phongMaterialNames[4] = {"Ka", "Kd", "Ks", "Shininess"};
//
//    GLuint gouradLightIndices[4], gouradMaterialIndices[4], gouradVertIndices[4];
//    GLuint phongVertIndices[4], phongLightIndices[4], phongMaterialIndices[4];
//
//    glGetUniformIndices(gouradShaderProgram->getShaderProgramHandle(), 4, gouradLightNames, gouradLightIndices);
//    glGetUniformIndices(gouradShaderProgram->getShaderProgramHandle(), 4, gouradMaterialNames, gouradMaterialIndices);
//    glGetUniformIndices(gouradShaderProgram->getShaderProgramHandle(), 4, gouradVertNames, gouradVertIndices);
//    glGetUniformIndices(phongShaderProgram->getShaderProgramHandle(), 4, phongVertNames, phongVertIndices);
//    glGetUniformIndices(phongShaderProgram->getShaderProgramHandle(), 4, phongLightNames, phongLightIndices);
//    glGetUniformIndices(phongShaderProgram->getShaderProgramHandle(), 4, phongMaterialNames, phongMaterialIndices);
//
//    glGetActiveUniformsiv(gouradShaderProgram->getShaderProgramHandle(), 4, gouradLightIndices,
//                          GL_UNIFORM_OFFSET, gouradLightOffsets);
//    glGetActiveUniformsiv(gouradShaderProgram->getShaderProgramHandle(), 4, gouradMaterialIndices,
//                          GL_UNIFORM_OFFSET, gouradMaterialOffsets);
//    glGetActiveUniformsiv(gouradShaderProgram->getShaderProgramHandle(), 4, gouradVertIndices,
//                          GL_UNIFORM_OFFSET, gouradVertOffsets);
//    glGetActiveUniformsiv(phongShaderProgram->getShaderProgramHandle(), 4, phongVertIndices,
//                          GL_UNIFORM_OFFSET, phongVertOffsets);
//    glGetActiveUniformsiv(phongShaderProgram->getShaderProgramHandle(), 4, phongLightIndices,
//                          GL_UNIFORM_OFFSET, phongLightOffsets);
//    glGetActiveUniformsiv(phongShaderProgram->getShaderProgramHandle(), 4, phongMaterialIndices,
//                          GL_UNIFORM_OFFSET, phongMaterialOffsets);
	...
	glBufferData( GL_ARRAY_BUFFER, NUM_CUBE_VERTICES * sizeof(GouradVertex), cubeVertices, GL_STATIC_DRAW );
//    glBufferData( GL_ARRAY_BUFFER, NUM_CUBE_VERTICES * sizeof(glm::vec3), vertexNormals, GL_STATIC_DRAW );
	...
	glEnableVertexAttribArray( gouradShaderAttributes.vPos );
	// map the vPos attribute to data within our buffer
	glVertexAttribPointer( gouradShaderAttributes.vPos, 3, GL_FLOAT, GL_FALSE, sizeof(GouradVertex), (void*) 0 );
    // enable our vColor attribute
    glEnableVertexAttribArray( gouradShaderAttributes.vNorm );
    // map the vColor attribute to data within our buffer
    glVertexAttribPointer( gouradShaderAttributes.vNorm, 3, GL_FLOAT, GL_FALSE, sizeof(GouradVertex), (void*) (3 * sizeof(GLfloat)) );
...
//
//    // enable our vPos attribute
//    glEnableVertexAttribArray( phongShaderAttributes.vPos );
//    // map the vPos attribute to data within our buffer
//    glVertexAttribPointer( phongShaderAttributes.vPos, 3, GL_FLOAT, GL_FALSE, sizeof(GouradVertex), (void*) 0 );
//    // enable our vColor attribute
//    glEnableVertexAttribArray( phongShaderAttributes.vNorm );
//    // map the vColor attribute to data within our buffer
//    glVertexAttribPointer( phongShaderAttributes.vNorm, 3, GL_FLOAT, GL_FALSE, sizeof(GouradVertex), (void*) (3 * sizeof(GLfloat)) );
    ...
    // map the vColor attribute to data within our buffer
    glVertexAttribPointer( colorShaderAttributes.vColor, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*) (3 * sizeof(GLfloat)) );
...
void setupScene() {
    ...
    glm::vec3 LightAmb = glm::vec3(0.1, 0.1, 0.1);
    glm::vec3 LightDiff = glm::vec3(1.0, 1.0, 1.0);
    glm::vec3 LightSpec = glm::vec3(0.5, 0.5, 0.5);
    gouradLightUBO->copyToOffset( 0, (void*)(&LightPos[0]), sizeof(glm::vec4) );
    gouradLightUBO->copyToOffset( 1, (void*)(&LightAmb[0]), sizeof(glm::vec3) );
    gouradLightUBO->copyToOffset( 2, (void*)(&LightDiff[0]), sizeof(glm::vec3) );
    gouradLightUBO->copyToOffset( 3, (void*)(&LightSpec[0]), sizeof(glm::vec3) );
    gouradLightUBO->bindBuffer();
    gouradLightUBO->bufferSubData();

//    phongLightUBO->copyToOffset( 0, (void*)(&LightPos[0]), sizeof(glm::vec4) );
//    phongLightUBO->copyToOffset( 1, (void*)(&LightAmb[0]), sizeof(glm::vec3) );
//    phongLightUBO->copyToOffset( 2, (void*)(&LightDiff[0]), sizeof(glm::vec3) );
//    phongLightUBO->copyToOffset( 3, (void*)(&LightSpec[0]), sizeof(glm::vec3) );
//    phongLightUBO->bindBuffer();
//    phongLightUBO->bufferSubData();
}
...
//    delete phongShaderProgram;
    ...
    delete gouradLightUBO;
    delete gouradMaterialUBO;
    delete gouradVertUBO;
//    delete phongVertUBO;
//    delete phongLightUBO;
//    delete phongMaterialUBO;
}
...
void renderScene( const glm::mat4 VIEW_MTX, const glm::mat4 PROJ_MTX ) {
    ...
    glm::mat4 modelviewMatrix = modelMtx * VIEW_MTX;

	// update our dynamic uniform values
//	glUniformMatrix4fv( colorShaderUniforms.mvpMtx, 1, GL_FALSE, &mvpMtx[0][0] );
    //TODO 1: Send data through UBOs, they all set up. Step 4 through 6 on page 59 in OpenGL cookbook
    //Data to send:
//    GLchar *gouradLightNames[4] = {"Position", "La", "Ld", "Ls"};
//    GLchar *gouradMaterialNames[4] = {"Ka", "Kd", "Ks", "Shininess"};
//    GLchar *gouradVertNames[4] = {"ModelViewMatrix", "NormalMatrix", "ProjectionMatrix", "MVP"};
...
//    GLchar *phongMaterialNames[4] = {"Ka", "Kd", "Ks", "Shininess"};
    ...
    glm::vec3 MaterAmb = glm::vec3(0.2, 0.2, 0.2);
    glm::vec3 MaterDiff = glm::vec3(1.0, 1.0, 1.0);
    glm::vec3 MaterSpec = glm::vec3(0.7, 0.7, 0.7);
    GLfloat shininess = 15.0f;
    glm::mat3 normalMtx = glm::transpose( glm::inverse(glm::mat3( modelMtx) ) );


    if (gouradShaderUsed && phongShading)
        gouradShaderSubroutineIndices.vertex = gouradShaderSubroutineIndices.vGouradPhong;
    else if (!gouradShaderUsed && phongShading) {
        gouradShaderSubroutineIndices.vertex = gouradShaderSubroutineIndices.vPhongPassThrough;
        gouradShaderSubroutineIndices.fragment = gouradShaderSubroutineIndices.fPhongPhong;
    }
    else if (gouradShaderUsed && !phongShading)
        gouradShaderSubroutineIndices.vertex = gouradShaderSubroutineIndices.vGouradBlinnPhong;
    else {
        gouradShaderSubroutineIndices.vertex = gouradShaderSubroutineIndices.vPhongPassThrough;
        gouradShaderSubroutineIndices.fragment = gouradShaderSubroutineIndices.fPhongBlinnPhong;
    }

    gouradShaderProgram->useProgram();
    glUniformSubroutinesuiv( GL_VERTEX_SHADER, 1,  &( gouradShaderSubroutineIndices.vertex ));
    glUniformSubroutinesuiv( GL_FRAGMENT_SHADER, 1,  &( gouradShaderSubroutineIndices.fragment ));

    gouradMaterialUBO->copyToOffset( 0, (void*)(&MaterAmb[0]), sizeof(glm::vec3) );
    gouradMaterialUBO->copyToOffset( 1, (void*)(&MaterDiff[0]), sizeof(glm::vec3) );
    gouradMaterialUBO->copyToOffset( 2, (void*)(&MaterSpec[0]), sizeof(glm::vec3) );
    gouradMaterialUBO->copyToOffset( 3, &shininess, sizeof(GLfloat) );
    gouradMaterialUBO->bindBuffer();
    gouradMaterialUBO->bufferSubData();

    gouradVertUBO->copyToOffset( 0, (void*)(&modelviewMatrix[0][0]), sizeof(glm::mat4) );
    gouradVertUBO->copyToOffset( 1, (void*)(&normalMtx[0][0]), sizeof(glm::mat3) );
    gouradVertUBO->copyToOffset( 2, (void*)(&PROJ_MTX[0][0]), sizeof(glm::mat4) );
    gouradVertUBO->copyToOffset( 3, (void*)(&mvpMtx[0][0]), sizeof(glm::mat4) );
    gouradVertUBO->bindBuffer();
    gouradVertUBO->bufferSubData();
//        phongShaderProgram->useProgram();

//        phongMaterialUBO->copyToOffset( 0, (void*)(&MaterAmb[0]), sizeof(glm::vec3) );
//        phongMaterialUBO->copyToOffset( 1, (void*)(&MaterDiff[0]), sizeof(glm::vec3) );
//        phongMaterialUBO->copyToOffset( 2, (void*)(&MaterSpec[0]), sizeof(glm::vec3) );
//        phongMaterialUBO->copyToOffset( 3, &shininess, sizeof(GLfloat) );
//        phongMaterialUBO->bindBuffer();
//        phongMaterialUBO->bufferSubData();
//
//        phongVertUBO->copyToOffset( 0, (void*)(&modelviewMatrix[0][0]), sizeof(glm::mat4) );
//        phongVertUBO->copyToOffset( 1, (void*)(&PROJ_MTX[0][0]), sizeof(glm::mat4) );
//        phongVertUBO->copyToOffset( 2, (void*)(&mvpMtx[0][0]), sizeof(glm::mat4) );
//        phongVertUBO->copyToOffset( 3, (void*)(&normalMtx[0][0]), sizeof(glm::mat3) );
//        phongVertUBO->bindBuffer();
//        phongVertUBO->bufferSubData();
//
//        glBindVertexArray( vaods[VAOS.CUBE_PHONG] );
//        glDrawElements( GL_TRIANGLES, iboCounts.cube_phong, GL_UNSIGNED_SHORT, (void*)0 );
